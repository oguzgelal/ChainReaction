...
public class creaction extends PApplet {
...
public void retry(int level_f) {
  level = level_f;
  clicked = 0;
  destination = (level*6);
}
public void next_level(int level_f) {
  level = level_f;
  clicked = 0;
  destination = (level*6);
}
...
public void manage(ball obj) { //manages the stages of objects
  if (obj.stage == 1) {
    obj.stage1();
    for (int i = infected_xy.size()-1; i>=0;i--) { //we used infected_x for control, but it doesnt matter because size of infexted_x and infected_y are the same. additions are done at the same time.
      ...
      for (int iii = 0; iii < ballnumber; iii = iii+1) {
        if ((myclickball.stage == 2) || (myclickball.stage == 3) || (myclickball.stage == 4)) {
          ...
          if ((myclickball.x == PApplet.parseFloat(infected_exp[0])) && (myclickball.y == PApplet.parseFloat(infected_exp[1]))) {
            if (dist(obj.x, obj.y, PApplet.parseFloat(infected_exp[0]), PApplet.parseFloat(infected_exp[1])) <= ((rd_var/2) + myclickball.rd/2)) {
              obj.stage = 2;
            }
          }
        }
        ...
        if ((createballs[iii].stage == 2) || (createballs[iii].stage == 3) || (createballs[iii].stage == 4)) {
          ...
          if ((createballs[iii].x == PApplet.parseFloat(infected_exp[0])) && (createballs[iii].y == PApplet.parseFloat(infected_exp[1]))) {
            if (dist(obj.x, obj.y, PApplet.parseFloat(infected_exp[0]), PApplet.parseFloat(infected_exp[1])) <= ((rd_var/2) + createballs[iii].rd/2)) { 
              obj.stage = 2;
            }
          }
        }
...
class ball { // clas---------------------------------------------------------------------
  ...
  public void stage1() { // before collapse | usage : ball.stage1();
    stage = 1;
    //arrange x
    if (x >= (width-(rd/2))) {
      carpx = -1;
  ...
  public void stage2(float x_f2, float y_f2) { //grow | usage : ball.stage2(ball.x,ball.y);
    stage = 2;
    //x,y freez
    x = x_f2;
    y = y_f2;
  ...
  public void stage3(float x_f3, float y_f3) { //stand | usage : ball.stage3(ball.x,ball.y);
    stage = 3;
    //x,y freez
    x = x_f3;
    y = y_f3;
  ...
  public void stage4(float x_f4, float y_f4) { //shrink | usage : ball.stage4(ball.x,ball.y);
    stage = 4;
    //x,y freez ()
    x = x_f4;
    y = y_f4;
  ...
  public void stage5(float x_f5, float y_f5) { //disappear | usage : ball.stage5();
    stage = 5;
    x = x_f5;
    y = y_f5;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--hide-stop", "creaction" };
    if (passedArgs != null) {
